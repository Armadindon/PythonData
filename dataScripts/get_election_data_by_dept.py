...
def main():
    ...
    Fonction Principale
    """
    file_dept = open(os.path.join('data', 'departements.csv'))
    dept_reader = DictReader(file_dept)

    fields = [
        "code_departement", "nom_departement", "parti_gagnant",
        "votes_MLEPEN",
    ...
    for commune in dept_reader:
        ...
        dept_data_request = requests.get("https://www.interieur.gouv.fr/" +
                                         "Elections/Les-resultats/Presidentielles/" +
                                         "elecresult__presidentielle-2017/" +
                                         "(path)/presidentielle-2017/" +
                                         format_number_of_char(
                                             commune["code_region"], 3)
                                         + "/" +
                                         format_number_of_char(
                                             commune["code_departement"], 3)
                                         + "/index.html")
        ...
        resultats_vote = html.fromstring(dept_data_request.content.decode("utf-8")
                                                 ...
                                                 ' tableau-resultats-listes"][2]/tbody/tr')
        parti_gagnant = ("", -1)
        ...
        for result_by_candidate in resultats_vote:
            ...
            if parti_gagnant[1] < int(data_by_candidate[1].replace(" ", "")):
                parti_gagnant = (parti_by_candidate[data_by_candidate[0]][0], int(
                    data_by_candidate[1].replace(" ", "")))

            result_item["votes_" +
                        parti_by_candidate[data_by_candidate[0]][1]] = int(
                data_by_candidate[1].replace(" ", ""))
            result_item["pourcentage_inscrit_" +
                        parti_by_candidate[data_by_candidate[0]][1]] = float(
                data_by_candidate[2].replace(",", "."))
            result_item["pourcentage_exprime_" +
                        parti_by_candidate[data_by_candidate[0]][1]] = float(
                data_by_candidate[3].replace(",", "."))
    ...
    with open(os.path.join('data', 'votes_departements.csv'),
